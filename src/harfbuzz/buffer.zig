...
pub const Buffer = struct {
    ...
    pub fn setContentType(self: Buffer, ct: ContentType) void {
        c.hb_buffer_set_content_type(self.handle, @backingInt(ct));
    }

    /// Fetches the type of buffer contents. Buffers are either empty, contain
    /// characters (before shaping), or contain glyphs (the result of shaping).
    pub fn getContentType(self: Buffer) ContentType {
        return @fromBackingInt(@intCast(c.hb_buffer_get_content_type(self.handle)));
    }
    ...
    pub fn setDirection(self: Buffer, dir: Direction) void {
        c.hb_buffer_set_direction(self.handle, @backingInt(dir));
    }

    /// See hb_buffer_set_direction()
    pub fn getDirection(self: Buffer) Direction {
        return @fromBackingInt(@intCast(c.hb_buffer_get_direction(self.handle)));
    }
    ...
    pub fn setScript(self: Buffer, script: Script) void {
        c.hb_buffer_set_script(self.handle, @backingInt(script));
    }

    /// See hb_buffer_set_script()
    pub fn getScript(self: Buffer) Script {
        return @fromBackingInt(@intCast(c.hb_buffer_get_script(self.handle)));
    }
...
test "create" {
    ...
    // Bind the dynamically loaded hb_* entry points before calling them.
    try @import("dyn.zig").ensureLoaded();
