...
test "loading memory font" {
    const testing = std.testing;
    const font_data = try @import("test.zig").loadTestFont(testing.allocator);
    defer testing.allocator.free(font_data);

    // Bind the dynamically loaded FT_* entry points before `Library.init`.
